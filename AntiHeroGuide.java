...
public class AntiHeroGuide 
{
	...
	public static void main(String[] args) 
    {
        ...
        System.out.print("\n\nChoose Mode: \n [1]ALL POSSIBLE COUNTER HEROES. \n [*]FILTERED COUNTER HEROES. \n\n CHOSEN MODE: ");
        int ans = sc.nextInt();
        if(ans == 1) {
            allPossibleAntiHeroes();
        } else {
            customizedAntiHeroes();
        }
	}
	...
	public static void allPossibleAntiHeroes() 
    {
		...
		while (true) 
        {
			...
			for (Hero hero: heroes) 
            {
				...
				for (String anti: goodAgainst) 
                {
					if (anti.contains("[" + heroNameInput + "]:")) 
                    {
						// Split the anti string into two parts, separated by ":"
						String[] parts = anti.split(":");
			...
			if (!foundAntiHero) 
        	{
				System.out.println("\nNo counter hero found in the Hero Database.");
			}
		}
	}

	public static void customizedAntiHeroes() 
    {
		Hero[] heroes = HeroDatabase.createAllHeroes();
		Scanner sc = new Scanner(System.in);
